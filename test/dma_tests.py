...
def dma_loopback(uri, classname, channel):
    ...
    start = 0
    step = 16
    tx_data = np.array(range(start, sdr.tx_ref // step), dtype=np.int16)
    tx_data = tx_data * step
...
def dds_two_tone(
    ...
    try:
        ...
        del sdr
        raise Exception(e)
    tone_peaks, tone_freqs = spec.spec_est(data, fs=RXFS, ref=sdr.rx_ref)
    del sdr
...
def t_sfdr(uri, classname, channel, param_set, sfdr_min, use_obs=False, full_scale=0.9):
    ...
    try:
        ...
        raise Exception(e)
    del sdr
    valr, amp, freqs, _, _, _ = spec.sfdr(data, plot=False)
